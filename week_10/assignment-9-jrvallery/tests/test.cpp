...
TEST_F(test_x, Test_isContaminated) {
    ...
    EXPECT_FALSE(d.isContaminated(1, 1));
    
    // Set (0,0) to radioactive.
    d.dungeon[0][0] = radioactive;
    // Cells adjacent to (0,0) should now be contaminated.
    EXPECT_TRUE(d.isContaminated(0, 1));
    EXPECT_TRUE(d.isContaminated(1, 1));
    // A cell far from (0,0) should remain uncontaminated.
    EXPECT_FALSE(d.isContaminated(2, 2));
    add_points_to_grade(10.0);
}
...
TEST_F(test_x, Test_findOpenAdjacentPaths) {
    ...
    vector<int> neighbors = d.findOpenAdjacentPaths(1, 1);
    EXPECT_EQ(neighbors.size(), 8);
    
    // Now mark (0,0) as radioactive. This should contaminate some neighbors.
    d.dungeon[0][0] = radioactive;
    neighbors = d.findOpenAdjacentPaths(1, 1);
